...
class Nodo(object):
	...
	safety_violated = False# set true if unsafe
        ...
        model = 'nonlinear_with_beta' #'nonlinear_with_beta', 'linear', 'nonlinear_with_beta'
	    ...
	    #####################simulation of malicious command#################
            ...
            if uncertainty_estimation_method == 'sampling':
                speed_bound = 0+1*0.04 #time invariant uncertainty of speed change in future 1s
                delta_bound = 0.002+1*0.27 #time invariant uncertainty of angle change in future 1s
            if uncertainty_estimation_method == 'gp':
                ...
                speed_bound = min(mu_vel+1*sigma_vel, 0+1*0.04)
                if speed_bound < 0+1*0.04:
                    counter_contract_speed+=1 
                    contract_ratio_speed += np.abs(speed_bound-0+1*0.04)
                ...
                delta_bound = min(mu_angle+1*sigma_angle, 0.002+1*0.27)
                if delta_bound < 0.002+1*0.27:
                    counter_contract_delta+=1 
                    contract_ratio_delta += np.abs(delta_bound-0.002-1*0.27)
...
def executeFlowstar(model, state, command, waypoint_x, waypoint_y, speed_bound, delta_bound):
    ...
    #uncertainty_speed = 0.001
    uncertainty_angle = (horizon/1)*delta_bound
    uncertainty_speed = (horizon/1)*speed_bound
    epsilon_cir = 0.1
    epsilon_line = 0.5
    ...
    #print delta_I[0][0], delta_I[0][1], 'delta'
