...
class Nodo(object):
    ...
    def flowCallback(self,flow_t):
        ...
        self.flow_x = flow_t.rectangle_y##
        self.flow_y = flow_t.rectangle_y2
        self.indicator =  flow_t.output_id
    def start(self,fig):
        while not rospy.is_shutdown() and len(self.flow_x_min.split(','))>0:
            x_list = []
            y_list = [] 
            ...
            if mode == "rounded_square":
            	...
            	plt.plot(np.arange(0, 1.1, 0.1), 0*np.arange(0, 1.1, 0.1), 'r', linewidth = 3.0 )#1st

            	th = np.arange(1.5*np.pi, 2*np.pi+np.pi/10, np.pi/10)
            	plt.plot(1 * np.cos(th)+1, 1 * np.sin(th)+1, 'r', linewidth = 3.0)#2nd

            	plt.plot(2+0*np.arange(1, 2.1, 0.1), np.arange(1, 2.1, 0.1), 'r', linewidth = 3.0)#3rd

            	th = np.arange(0*np.pi, 0.5*np.pi+np.pi/10, np.pi/10)
            	plt.plot(1 * np.cos(th)+1, 1 * np.sin(th)+2, 'r', linewidth = 3.0)#4th

            	plt.plot(np.arange(0, 1.1, 0.1), 3+0*np.arange(0, 1.1, 0.1), 'r', linewidth = 3.0)#5th               

            	th = np.arange(0.5*np.pi, 1.0*np.pi+np.pi/10, np.pi/10)
            	plt.plot(1 * np.cos(th)+0, 1 * np.sin(th)+2, 'r', linewidth = 3.0)#6th

            	plt.plot(-1+0*np.arange(0, 1.1, 0.1), np.arange(1, 2.1, 0.1), 'r', linewidth = 3.0)#7th

            	th = np.arange(np.pi, 1.5*np.pi+np.pi/10, np.pi/10)
            	plt.plot(1 * np.cos(th)+0, 1 * np.sin(th)+1, 'r', linewidth = 3.0)#8th
                plt.xlim(-2, 5)
                plt.ylim(-2, 5)
            if mode == "straight":
                plt.plot(initial_x, initial_y, 'b*', markersize=12)
                plt.plot(np.arange(0, 10, 0.1), 0*np.arange(0, 10, 0.1), 'r', linewidth = 3.0 )
                plt.xlim(-1, 10)
                plt.ylim(-1, 1)
            if mode == "circle":
                plt.plot(initial_x, initial_y, 'b*', markersize=12)  
            	th = np.arange(0*np.pi, 2*np.pi+np.pi/10, np.pi/10)
            	plt.plot(1 * np.cos(th), 1 * np.sin(th)+1, 'r', linewidth = 3.0)
                plt.xlim(-2, 2)
                plt.ylim(-1, 3)
            fig.canvas.draw()
            plt.clf()
