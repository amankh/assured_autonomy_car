...
static void subject_publish_callback(const ViconDriver::Subject &subject)
{
  ...
  if(loadCalib(subject.name))
  {
    ...
    const Eigen::Vector3d calib_position(calib_pose[subject.name].translation());
    ...
    subject_ros.position.x = position.x() + calib_position.x();
    subject_ros.position.y = position.y() + calib_position.y();
