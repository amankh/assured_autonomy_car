...
ViconOdom::ViconOdom(ros::NodeHandle &nh)
{
  ...
  odom_pub_ = nh.advertise<nav_msgs::Odometry>("vicon_odom", 10);
  odom_body_pub_ = nh.advertise<nav_msgs::Odometry>("vicon_odom_body", 10);
...
void ViconOdom::ViconCallback(const vicon::Subject::ConstPtr &msg)
{
  ...
  odom_body_msg.header = odom_msg.header;
  odom_body_msg.child_frame_id = odom_msg.child_frame_id;
  odom_body_msg.pose = odom_msg.pose;
  Eigen::MatrixXd spatial_twist_(3,1);
  Eigen::MatrixXd body_twist_(3,1);
  spatial_twist_(0,0) = state(3);
  spatial_twist_(1,0) = state(4);
  spatial_twist_(2,0) = state(5);
  ...
  if(dt > 1e-6)
  {
    ...
    // Computing Body Twist  
    const Eigen::Matrix3d w_hat_body = R.transpose() * R_dot;
   
    odom_body_msg.twist.twist.angular.x = w_hat_body(2, 1);
    odom_body_msg.twist.twist.angular.y = w_hat_body(0, 2);
    odom_body_msg.twist.twist.angular.z = w_hat_body(1, 0);

    
  }
  R_prev = R;

  //const Eigen::MatrixXd body_twist_ = R.transpose() * spatial_twist_;
  body_twist_ = R.transpose() * spatial_twist_;
  odom_body_msg.twist.twist.linear.x = body_twist_(0,0);
  odom_body_msg.twist.twist.linear.y = body_twist_(1,0);
  odom_body_msg.twist.twist.linear.z = body_twist_(2,0);


  odom_pub_.publish(odom_msg);
  odom_body_pub_.publish(odom_body_msg);
