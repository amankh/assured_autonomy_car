...
// exp1 : steering angle from -30 to 0 
// exp1 : speed 1.5
...
int main(int argc, char **argv)
{
    ...
    outfile.open("/home/nvidia/Desktop/Sysid/ramp_steer_exp1_trial1.csv");
    // left turn ramp steering test
    for(int steering_ang_deg = -10; steering_ang_deg <= 10 && ros::ok() ; steering_ang_deg ++)
    {
        cmd.drive.speed = forward_vel;
        cmd.drive.steering_angle = steering_ang_deg*M_PI/180.0;

        cmd_pub.publish(cmd);
        ROS_INFO("ramp_steer_exp1: Running at steering angle %d degrees.\n", steering_ang_deg);
        ROS_INFO("ramp_steer_exp1: Running at forward velocity %lf .\n", forward_vel);

        ros::Time start_time = ros::Time::now();
        while ( (ros::Time::now() - start_time).toSec() < 5.0 && ros::ok() ) {
