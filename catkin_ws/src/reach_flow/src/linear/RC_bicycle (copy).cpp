...
int main()
{
	...
	int v_id = stateVars.declareVar("v");
        int av_id = stateVars.declareVar("av");
	...
	Expression_AST<Real> ode_expression_v("0");//av*[-0.1,0.1]
        Expression_AST<Real> ode_expression_av("0");
	...
	ode_rhs[v_id] = ode_expression_v;
        ode_rhs[av_id] = ode_expression_av; 
...
//	setting.setAdaptiveStepsize(0.01, 0.04, 5);
	...
	Interval init_x(1, 1.01), init_y(-0.01, 0.01), init_psi(-0.01, 0.01),
			init_beta(-0.001, 0.001), init_v(1.0, 1.0), init_av(1, 1), init_t;
