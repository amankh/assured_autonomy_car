...
// exp2 : steering angle from -30 to 0 
// exp2 : speed 1
...
int main(int argc, char **argv)
{
    ...
    outfile.open("/home/nvidia/Desktop/Sysid/ramp_steer_exp2_trail0.csv");
    // left turn ramp steering test
    for(int steering_ang_deg = -30; steering_ang_deg <= 0 && ros::ok() ; steering_ang_deg ++)
    {
        cmd.drive.speed = forward_vel;
        cmd.drive.steering_angle = steering_ang_deg*M_PI/180.0;

        cmd_pub.publish(cmd);
        ROS_INFO("ramp_steer_exp2: Running at steering angle %d degrees.\n", steering_ang_deg);
        ROS_INFO("ramp_steer_exp2:Running at forward velocity %lf .\n", forward_vel);
