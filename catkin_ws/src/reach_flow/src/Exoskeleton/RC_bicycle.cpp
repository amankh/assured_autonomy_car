...
int main()
{
	...
	int q3_id = stateVars.declareVar("q3");
	int q4_id = stateVars.declareVar("q4");
	int q5_id = stateVars.declareVar("q5");
        int q3dot_id = stateVars.declareVar("q3dot");
        int q4dot_id = stateVars.declareVar("q4dot");
        int q5dot_id = stateVars.declareVar("q5dot");
	...
	//Expression_AST<Real> ode_expression_vy("-psidot * vx + 1/m * (F_xr*cos(delta)+F_yr)");
        Expression_AST<Real> ode_expression_psidot("-2.15*Fyr+2.45*Fyf");//1/I_z * (l_f*F_yf-l_r*F_yr),17.92 * (0.137*Fyf-0.12*Fyr), 2.45*Fyf-2.15*Fyr           
...
//	setting.setAdaptiveStepsize(0.01, 0.04, 5);
	...
	Interval init_x(0.017, 0.02), init_y(-0.11, -0.08), init_psi(-0.01, 0.01),
			init_psidot(0, 0.001), init_Fxf(0.0, 0,0), init_Fxr(0,0, 0.0), init_Fyf(0.01, 0.02), init_Fyr(0.1, 0.11), init_beta(0.01, 0.02), init_v(0.61, 0.71), init_t;
